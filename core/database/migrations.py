...
def create_tables():
    ...
    indexes_sql = [
        ...
        "CREATE INDEX IF NOT EXISTS idx_staging_status ON staging_questions(status)",
        "CREATE INDEX IF NOT EXISTS idx_staging_created ON staging_questions(created_at)",
    ...
    try:
        ...
        db.execute(staging_questions_sql)
        db.execute(qa_logs_sql)
...
def migrate_database(auto: bool = True):
    ...
    try:
        ...
        tables = ["categories", "tags", "questions", "question_tags", "staging_questions", "qa_logs"]
        missing_tables = []
